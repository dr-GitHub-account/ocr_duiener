...
#                                             cache_dir=args.cache_dir if args.cache_dir else None, )
    ...
    # **作用是得到一个字典,字典中元素的先后顺序取决于你的环境
    # **收集调用时的关键字参数(该情况下即do_lower_case, cache_dir)到一个字典里
    # *收集调用时剩余的参数到一个元组里
    @classmethod
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        # 在tokenization_bert.py中定义max_model_input_sizes为PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES
        # 这是一个包含所有常用模型到各自最大输入的映射的字典
        ...
        # 默认进入下面的else
            ...
            for file_id, file_name in cls.vocab_files_names.items():
                # 默认进入下面的if，full_file_name为.../chinese-roberta-wwm-ext/vocab.txt
                ...
                # vocab.txt是存在的，默认不进入下面的if
                if not os.path.exists(full_file_name):
                    logger.info("Didn't find file {}. We won't load it.".format(full_file_name))
                    full_file_name = None
                # vocab_files = {'vocab_file': '.../chinese-roberta-wwm-ext/vocab.txt'}
            ...
            # 查找saved_directory(即pretrained_model_name_or_path)中，
            # 是否有added_tokens.json/special_tokens_map.json/tokenizer_config.json
            # 若有的话，就将vocab_files中'added_tokens_file'/'special_tokens_map_file'/'tokenizer_config_file'
            # 键对应的值设为文件路径，否则值设为None
        ...
        # vocab_files = {
                        ...
                        # 'tokenizer_config_file': '.../chinese-roberta-wwm-ext/tokenizer_config.json'
                        # }
        try:
        ...
        # 日志中记录加载vocab_file, added_tokens_file, special_tokens_map_file, tokenizer_config_file的信息
        for file_id, file_path in vocab_files.items():
        ...
        # 默认进入下面的if，tokenizer_config_file = '.../chinese-roberta-wwm-ext/tokenizer_config.json'
        if tokenizer_config_file is not None:
            # 加载tokenizer_config.json文件，文件内容：
            # {"init_inputs": []}
        ...
        # logger.info("**************BEFORE-kwargs: {}**************".format(kwargs))
        # logger.info("**************BEFORE-init_kwargs: {}**************".format(init_kwargs))
        init_kwargs.update(kwargs)
        # logger.info("**************FTER-kwargs: {}**************".format(kwargs))
        # logger.info("**************AFTER-init_kwargs: {}**************".format(init_kwargs))
        # # **************BEFORE-kwargs: {'do_lower_case': True}**************
        # # **************BEFORE-init_kwargs: {}**************
        # # **************AFTER-kwargs: {'do_lower_case': True}**************
        # # **************AFTER-init_kwargs: {'do_lower_case': True}**************

        # Set max length if needed，默认不进入下面的if
        ...
        # added_tokens_file = None, special_tokens_map_file = None
        ...
        # Instantiate tokenizer of class PreTrainedTokenizer
        # def __init__(self, max_len=None, **kwargs):
        #     self._bos_token = None
        #     self._eos_token = None
        #     self._unk_token = None
        ...
        # print("*******************tokenizer.init_inputs: {}*******************".format(tokenizer.init_inputs))
        # print("*******************tokenizer.init_kwargs: {}*******************".format(tokenizer.init_kwargs))
        # # *******************tokenizer.init_inputs: []*******************
        # # *******************tokenizer.init_kwargs: {'do_lower_case': True, 'vocab_file': '/home/user/xiongdengrui/ocr_duiener/pytorch_version/prev_trained_model/chinese-roberta-wwm-ext/vocab.txt'}*******************

        # Add supplementary tokens. 默认不进入下面的if
